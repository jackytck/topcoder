...
// END CUT HERE
#line 114 "MeasuringTemperature.cxx"
#include <vector>
#include <list>
#include <map>
#include <set>
...
class MeasuringTemperature {
	public:
		...
		{		
			vector <int> valid;
			for(int i=0; i<measuredValues.sz; i++)
			{
				bool val = false;
				if(i-1 >= 0 && abs(measuredValues[i]-measuredValues[i-1]) <= 2)
					val = true;
				if(i-2 >= 0 && abs(measuredValues[i]-measuredValues[i-2]) <= 2)
					val = true;
				if(i+1 < measuredValues.sz && abs(measuredValues[i]-measuredValues[i+1]) <= 2)
					val = true;
				if(i+2 < measuredValues.sz && abs(measuredValues[i]-measuredValues[i+2]) <= 2)
					val = true;
				if(val && measuredValues[i] > -274)
					valid.PB(measuredValues[i]);
			}
			return valid.sz == 0 ? double(-300.) : double(accumulate(valid.begin(), valid.end(), 0)) / valid.sz;
		}
};
